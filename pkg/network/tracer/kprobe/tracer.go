...
func (t *kprobeTracer) GetConnections(buffer []network.ConnectionStats, filter func(*network.ConnectionStats) bool) ([]network.ConnectionStats, error) {
	...
	key, stats := &netebpf.NamespacedConnTuple{}, &netebpf.ConnStats{}
	seen := make(map[netebpf.NamespacedConnTuple]struct{})
	entries := t.conns.IterateFrom(unsafe.Pointer(&netebpf.NamespacedConnTuple{}))
...
func (t *kprobeTracer) Remove(conn *network.ConnectionStats) error {
	...
	entry := &netebpf.NamespacedConnTuple{
		Sport: conn.SPort,
		Dport: conn.DPort,
		Netns: conn.NetNS,
		Pid:   conn.Pid,
	}
	entry.Saddr_l, entry.Saddr_h = util.ToLowHigh(conn.Source)
	entry.Daddr_l, entry.Daddr_h = util.ToLowHigh(conn.Dest)

	if conn.Family == network.AFINET6 {
		entry.Metadata |= uint32(netebpf.IPv6)
	} else {
		entry.Metadata |= uint32(netebpf.IPv4)
	}
	if conn.Type == network.TCP {
		entry.Metadata |= uint32(netebpf.TCP)
	} else {
		entry.Metadata |= uint32(netebpf.UDP)
	}

	err := t.conns.Delete(unsafe.Pointer(entry))
	if err != nil {
	...
	_ = t.tcpStats.Delete(unsafe.Pointer(entry))
	entry.Pid = pid
...
func (t *kprobeTracer) getTCPStats(tuple *netebpf.NamespacedConnTuple, seen map[netebpf.NamespacedConnTuple]struct{}) *netebpf.TCPStats {
...
func connStats(t *netebpf.NamespacedConnTuple, s *netebpf.ConnStats, tcpStats *netebpf.TCPStats) network.ConnectionStats {
