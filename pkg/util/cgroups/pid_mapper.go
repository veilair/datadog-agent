...
type pidMapper interface {
	getPIDsForCgroup(relativeCgrouPath string, cacheValidity time.Duration) []int
}
	...
	if err == nil {
		...
		if err != nil && len(content) > 0 {
			return &cgroupProcsPidMapper{
				...
				cgroupProcsFilePathBuilder: func(relativeCgrouPath string) string {
					return filepath.Join(cgroupRoot, baseController, relativeCgrouPath, cgroupProcsFile)
				},
			}
		}
	}
...
func (pm *cgroupProcsPidMapper) getPIDsForCgroup(relativeCgrouPath string, cacheValidity time.Duration) []int {
	var pids []int

	if err := parseFile(pm.fr, pm.cgroupProcsFilePathBuilder(relativeCgrouPath), func(s string) error {
...
func (pm *procPidMapper) getPIDsForCgroup(relativeCgrouPath string, cacheValidity time.Duration) []int {
	pm.lock.Lock()
	defer pm.lock.Unlock()

	pm.refreshMapping(cacheValidity)
	return pm.cgroupPidsMapping[relativeCgrouPath]
}
